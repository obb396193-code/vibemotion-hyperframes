...
> **⚠️ 两个轴正交，别绑死（重要）**：
> - **轴一 = 主题（外观）**：只管**颜色 + 字体 + 调性**——上面三套就是三组 token。
> - **轴二 = 渲染/动效技术（能力）**：怎么渲、怎么动——Route B 真界面 / registry 现成零件(代码块·字幕·转场) / three.js 液态玻璃·3D / Lottie 贴片 / GSAP 插件 / HyperShader 转场。
> - **技术不属于任何主题**。「液态玻璃」「会动的代码块」是**渲染方式**，颜色是**喂给它的参数**——紫色片喂紫色 token，暖色片喂暖橙 token。别写成"紫色主题的液态玻璃"，要写成"液态玻璃技术 + 当前主题的色板"。
> - **用法 = 任选 1 个主题 ×（任意几个技术）自由组合**。主题决定"什么颜色/什么调"，技术决定"画面怎么渲怎么动"，两者各调各的。
...
## 🧭 选题模糊时：用「选择题」一步步带用户选（选着选着就看见都有啥）
> 用户只给笼统选题（"做个讲 AI 的""讲讲 Skill"）时，**别开放式反问、也别一次性长问**。把每个决策做成一道**选择题**——用 `AskUserQuestion`，**选项 = 库里真有的具体东西**，用户选一道、发下一道，**选来选去自然就看见有哪些可挑**。能从内容直接定的就别问、直接定 + 告知；真要用户拍板的才出选择题。

**选择题顺序（每题 3-4 个具体选项，可一次发多题）：**
1. **路线/方向**：界面库路线(真界面演操作·直观) / 代码块路线(讲代码·偏技术) / 像素包装(轻松涨粉) / 纯文字钩子。
2. **主题(颜色)**：🟣 紫色镀铬(酷炫包装) / 🟠 暖色衬线(沉静科普) / 🟦 像素(游戏感)。
3. **画幅**：竖屏 1080×1920(抖音) / 横屏 1920×1080(B站) / 方屏。
4. **每场用啥素材**（按场逐个出选择题，选项报**真名**）：
   - 要真界面 → 🖥️ **界面库**选：Claude Code 对话 / 终端 / VS Code 编辑器 / 浏览器 / GitHub PR / 4工具同屏…（38 个，`assets/ui-library/README.md`）
   - 要代码 → ⌨️ **代码块**选：打字 `code-typing` / 逐行高亮 `code-highlight` / 红删绿增 `code-diff` / 形变 `code-morph` + **25 个 VS Code 皮** + 卡点字幕 `caption-*`
   - 要套路 → 🎬 **配方 8 个**选：质感发布 / 竖屏带货 / AI对话 / 合集漂浮卡 / 技法菜单…
   - 要加料 → Lottie / GSAP 插件 / shader 转场 / three.js 液态玻璃
5. 选完 → 列分场清单进下面的 🔴 CHECKPOINT 终确认，再开渲。
> **要点**：选项必须是**真实可选项**（报得出文件名 / `add` 名），让用户在"看得见的菜单"里挑；每个选项旁标一句"偏直观/偏技术/偏轻松"帮判断。库里不够 → 选项里留一个"都不满意，去找/扒新的"。

## 做一条视频的流程

1. **选主题 + 选画幅**（**选题模糊 → 先走上面「🧭 选择题」一步步带用户选定方向/主题/画幅/素材，再进本流程**）
...
3. **要素材 / 现成件?**（先翻库复用，别从零做——详见下方「做片思路库」索引）
   - **真界面**（Claude Code 对话/终端/VS Code/浏览器/PR…）→ 先翻 `assets/ui-library/`（38 个现成真界面，直接复用，或 `data-composition-src` 多个组合成片）；库里没有 → `references/realistic-ui-and-web.md` 的 **Route B**（扒真站/复刻，含规范来源 A/B/C；另有「截图合成」管线备选）。
   - **会动代码块 / 卡点字幕 / shader 转场**→ `references/registry-parts.md`（缓存 18 个 + `npx hyperframes add <名>` 拿更多 ~130）。
   - **Lottie 贴片**（思考/loading…）→ `references/lottie.md`。
   - **不知怎么编排** → `references/official-recipes.md`（8 视觉配方）+ `references/storyboard-pacing.md`（节奏分镜）。
   - **logo / 精灵图 / 字体** → `references/asset-sourcing.md`，联网找 CC0 并本地化、记来源。
...
| **镀铬/标题整个不见**（父层挡住） | 给标题**父容器**加了 `opacity:0` 兜底，但 `PremKit.chromeIn`/charIn 只动**子元素**(`.chrome`/字符)的 autoAlpha，父层 opacity:0 没人开 | 别给父容器 opacity:0；改 `tl.set('#s .chrome',{autoAlpha:0},0)` 兜底隐藏**揭示动画实际作用的那一层**，再由 chromeIn 揭示 | 核对每个"兜底 opacity:0"的元素，是否正是动画要揭示的那一层 |
| **嵌套子合成 lint 报 `multiple_root_compositions`** | 子合成 `.html` 和 master `index.html` 都带根级 `data-composition-id`，放同级被当成两个根 | 子合成挪进 `compositions/` 子目录，master 用 `data-composition-src` 指过去；slot 的 `data-composition-id` **必须 = 子文件内部 div 的 id**（否则白屏） | 确认 master 只有一个根 `data-composition-id` |
...
## 做片思路库（规划时按需翻 —— 都来自官方成片/源码提炼）
- `references/storyboard-pacing.md` —— **节奏与分镜方法论**（官方 STORYBOARD 提炼：故事骨架/场要短/转场要省/连续性装置/对比即结构）+ 分场清单模板 + 自愈渲染循环。
- `references/official-recipes.md` —— **可复用视觉配方**（质感发布片 / 卡点字幕 / AI 对话演示 / kinetic typography / UI 功能演示）+ 官方"贵"的 5 条通用原则。
- `references/realistic-ui-and-web.md` —— **拟真界面**（Route B 真界面动画 + 规范来源 A/B/C + 必停问用户来源）。
- `references/ui-spec-standard.md` —— **扒新界面的规范标准**（8 块模板：色/字/图标/cqw/结构/动效）。
- `assets/ui-library/` —— **现成真界面库**（38 个真界面：Claude Code 对话×18/终端/VS Code/浏览器/PR/工具…，可直接复用 + 用 `data-composition-src` 组合成片，见库内 `example-master.html`）。
- `references/registry-parts.md` + `assets/registry-parts/` —— **官方现成零件**：会动的代码块(`code-typing/highlight/diff`)、卡点字幕(`caption-*`)；`npx hyperframes add <名>` 拿更多。
- `references/lottie.md` + `assets/lottie/` —— **Lottie 动画零件**（真 Claude 思考动画等；HF 原生支持，范例 `examples/lottie-thinking/` 已验证）。
- `assets/design-templates/` —— **open-design 设计规范富矿**（18 个科普/HF 向 DESIGN.md，色板/版式参考；入主题库前先渲染验证）。
- `assets/reference-scenes/` —— **官方写法范本**（思考场布局 / kinetic 文字 / three.js 参考；研究写法用，非直接复用件）。
> 都是「验证过的起点」，不是上限：不够就按各文档写的去找/扒/生成/`add`。
